...
class DDPGAgent(BaseAgent):
    def __init__(self,
     ...
     leaning_rate_critic=0.001):
        ...
        policy_gradient = tf.gradients(self.critic_behaviour.output, self.critic_behaviour.input)[0][:,4:5]
        actor_gradient = tf.gradients(self.actor_behaviour.output, self.actor_behaviour.trainable_variables, -policy_gradient)
        
        self.train_actor = tf.train.AdamOptimizer(self.learning_rate_actor).apply_gradients(zip(actor_gradient, self.actor_behaviour.trainable_variables))
    ...
    def act(self, state):
        ...
        action = prediction[0]+np.random.normal(scale=0.1)
    ...
    def train(self, state, action, reward: float, next_state, done: bool):
        ...
        session.run([self.train_actor], {
            self.critic_behaviour.input: np.hstack((batch.states_before, batch.actions)),
    ...
    def setup_actor(self):
        ...
        self.actor_behaviour.add(Dense(50, input_dim=self.state_space.shape[0], kernel_initializer='normal', activation='tanh'))
    ...
    def setup_critic(self):
        ...
        self.critic_behaviour.add(Dense(50, input_dim=self.state_space.shape[0]+self.action_space.shape[0] , kernel_initializer='normal', activation='tanh'))
